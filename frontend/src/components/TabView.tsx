...
import {
  dataState as dataStateAtom,
  viewData as viewDataAtom,
  RefContext,
} from "./atoms";
...
function TabView() {
  ...
  const [isImportTabOpen, setImportTabOpen] = useState(allTabs.has("Import"));
  ...
  async function fetchData(filename: string) {
    setIsLoading(true);
    const result = data.get(filename);
    if (result === undefined) {
  ...
  function closeTab(fileName: string, e: Event | null): void {
    ...
    if (len === 1) {
      ...
      if (activeTab === fileName) {
        if (fileName === "Import") {
          ...
          let currentIndex = key.indexOf(fileName);
          let newTab = key[currentIndex - 1];
          //rekurzija ko zaprem import tab
          if (newTab === undefined && fileName !== "Import") {
            newTab = key[currentIndex + 1];
          }
          activateTab(newTab);
        }
      }
    ...
    //remove from state
    if (data.delete(fileName)) setData(data);
        ...
        <div className="tabview">
          <div className="tabs" key={randomKey}>
            <>
              ...
              {allTabs.size === 0
                ? importTab()
                : Array.from(allTabs).map(([key, value], index) => {
                    return (
                      <TabChild
                        key={key}
                        tabData={value}
